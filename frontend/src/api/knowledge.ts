...
// 지식베이스 문서공개 여부 수정
export async function putDocKnowledge(knowledgeId: string, data: { title: string; isPublic: boolean }): Promise<KnowledgeData> {
  try {
...
// 지식베이스 청크 상세보기
export async function getChunkDetail(knowledgeId: string, chunkId: string): Promise<ChunkList> {
  try {
...
// 지식베이스 청크 수정
export async function postChunkDetail(knowledgeId: string, chunkId: string, data: { content: string }): Promise<boolean> {
  try {
